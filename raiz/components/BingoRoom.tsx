...
const BingoRoom: React.FC = () => {
  const { 
    gameState, user, userCards, ranking, buySeries, adminConfig, logout, seriesPrice, deposit
  } = useGame();
  ...
  const handleBuy = async () => {
    const success = await buySeries(buyQuantity);
    if (success) {
      alert(`${buyQuantity} Série(s) adquirida(s)! Você recebeu ${buyQuantity * 6} cartelas.`);
  ...
  return (
    <div className="min-h-screen lg:h-screen flex flex-col bg-[#1F1F3D] overflow-hidden relative">
      ...
      <div className="h-[55%] flex-none relative z-10 p-4 flex flex-col md:flex-row gap-4 overflow-hidden">
        <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 pb-8">
          <div className="flex justify-between items-center mb-4 sticky top-0 bg-black/40 py-2 backdrop-blur-md rounded-xl px-4 border border-white/5 z-20">
            <h3 className="text-sm font-black text-white/60 uppercase tracking-widest">Sua Mesa ({userCards.length} Cartelas)</h3>
            <div className="flex items-center gap-2">
      ...
      {/* Modal Compra de Séries (Redesigned with Volume) */}
      {showBuyModal && (
        <div className="fixed inset-0 bg-black/80 z-[60] flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
          <div className="bg-[#1F1F3D] rounded-[2.5rem] w-full max-w-sm overflow-hidden shadow-2xl border border-white/10 animate-pop">
            <div className="p-8 text-center border-b border-white/5 bg-gradient-to-b from-white/5 to-transparent">
              <h2 className="text-[#00D084] font-black text-2xl uppercase tracking-tighter">Comprar Séries</h2>
              <p className="text-gray-400 text-[10px] font-bold uppercase mt-2">Cada série contém 6 cartelas</p>
            </div>
            
            <div className="p-10 space-y-8">
              <div className="flex flex-col items-center">
                <div className="text-[10px] font-black text-gray-500 uppercase mb-4 tracking-widest">Quantas Séries deseja?</div>
                <div className="flex items-center gap-6">
                  <button 
                    onClick={() => setBuyQuantity(Math.max(1, buyQuantity - 1))}
                    className="w-14 h-14 rounded-2xl bg-white/5 flex items-center justify-center text-white font-black text-2xl hover:bg-white/10 transition border border-white/10"
                  >
                    -
                  </button>
                  <div className="flex flex-col items-center">
                    <div className="text-6xl font-black text-white leading-none">{buyQuantity}</div>
                    <div className="text-[10px] font-black text-[#9D4EDD] uppercase mt-2">Séries</div>
                  </div>
                  <button 
                ...
                <div className="mt-6 bg-[#00D084]/10 px-6 py-2 rounded-full">
                   <div className="text-[#00D084] text-[12px] font-black uppercase tracking-tighter">
                      Total de {buyQuantity * 6} Cartelas
                   </div>
                </div>
              </div>
              ...
              <div className="bg-black/20 p-6 rounded-3xl border border-white/5 space-y-2">
                <div className="flex justify-between text-[10px] font-black text-gray-500 uppercase">
                   <span>Preço da Série</span>
                   <span className="text-white">R$ {seriesPrice.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-xl font-black text-white uppercase border-t border-white/5 pt-3 mt-1">
                   <span className="text-[#00D084]">Total Pagar</span>
                   <span className="text-[#00D084]">R$ {(seriesPrice * buyQuantity).toFixed(2)}</span>
                </div>
              </div>

              <button 
                onClick={handleBuy}
                className="w-full bg-[#00D084] py-6 rounded-2xl text-white font-black text-sm shadow-xl shadow-[#00D084]/20 hover:scale-[1.02] transition uppercase tracking-widest"
              >
                Confirmar e Comprar
              </button>
              
              <div className="flex flex-col items-center gap-2">
                <div className="text-[10px] font-black text-gray-500 uppercase">Seu Saldo: <span className="text-white">R$ {user?.balance.toFixed(2)}</span></div>
                <button onClick={() => setShowBuyModal(false)} className="text-gray-500 font-black text-[9px] uppercase mt-4 hover:text-red-400 transition">Fechar</button>
              </div>
            </div>
          </div>
        </div>
