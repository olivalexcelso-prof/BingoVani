...
export const GameProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const buySeries = async (id: string) => {
    ...
    setUsers(p => p.map(u => u.id === user.id ? { ...u, balance: newBal } : u));
    setTransactions(p => [...p, { id: Math.random().toString(), userId: user.id, type: 'PURCHASE', amount: cfg.price, description: `Série ${id}`, status: 'COMPLETED', date: new Date().toISOString() }]);
    
    const cards: Card[] = Array.from({ length: cfg.quantity }, () => ({
      id: Math.random().toString(36).substr(2, 5).toUpperCase(),
      userId: user.id,
      numbers: Array.from({ length: 3 }, () => Array.from({ length: 9 }, () => Math.floor(Math.random() * 90) + 1)),
      marked: []
    }));
    setUserCards(p => [...p, ...cards]);
    return true;
  };
